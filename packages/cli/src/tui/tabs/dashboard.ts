...
export function renderDashboardTab(snapshot: GameState): string[] {
	...
	const headline = [
		...
		`Active contracts: ${liveContracts.length}`,
		`Market contracts: ${selectOpenMarketContractsFromState(snapshot).length}`,
	];
	...
	if (liveContracts.some((contract) => contract.lifecycleState === "breached")) {
